...
int main(int argc, char **argv[])
{
	...
	outArray = (double **) malloc((nobj/2+1)*sizeof(double *));
	for(i=0;i<(nobj/2+1);i++){
		outArray[i]  = (double *) malloc(11*sizeof(double));
	}
	
	outArray2 = (double **) malloc((nobj/2+1)*sizeof(double *));
	for(i=0;i<(nobj/2+1);i++){
		outArray2[i]  = (double *) malloc(11*sizeof(double));
	}
	...
	for(i = 0; i < nobj; i++){
		if ((fabs(body[i].z) < (body[i].h*2.0))){
			if (body[i].ident >= id) {
				...
				outArray2[j][0] = x;
				outArray2[j][1] = rho;
				outArray2[j][2] = body[i].temp;
				outArray2[j][3] = mass;
				outArray2[j][4] = pressure;
				outArray2[j][5] = cs;
				outArray2[j][6] = vx;
				outArray2[j][7] = u;
				outArray2[j][8] = (body[i].C12 + body[i].O16)/2;
				outArray2[j][9] = body[i].Si28;
				outArray2[j][10] = body[i].Ni56;
				j++;
			}
		}
	}
	...
	for(i = 0; i < j; i++)
	{
		if (outArray2[i][8] > outArray2[i][9] && outArray2[i][8] > outArray2[i][10]){
			if (i % 5 == 0) {
				outArray2[i][8]	= outArray2[i][2];
				outArray2[i][9]	= 1;
				outArray2[i][10] = 1;
			} else {
				outArray2[i][8]	= 1;
		...
		} else if (outArray2[i][9] > outArray2[i][8] && outArray2[i][9] > outArray2[i][10]){
			outArray2[i][8]	= 1;
			outArray2[i][9]	= outArray2[i][2];
			outArray2[i][10] = 1;
		} else {
			outArray2[i][8]	= 1;
			outArray2[i][9]	= 1;
			outArray2[i][10] = outArray2[i][2];
		}
	}
	
	
	//open the stream file
	snprintf(csvfile, sizeof(csvfile), "%s_rhoT.csv", argv[1]);
	...
	snprintf(csv2file, sizeof(csv2file), "%s_rhoT2.csv", argv[1]);
	/* declare a stream and prototype fopen */ 
	...
	for(i = 0; i < j; i++)
	{
		fprintf(stream,"%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f\n", outArray2[i][0],outArray2[i][1],outArray2[i][2],
				outArray2[i][3],outArray2[i][4],outArray2[i][5],outArray2[i][6],outArray2[i][7],
				outArray2[i][8],outArray2[i][9],outArray2[i][10]);
	}
	fclose(stream);
	...
	fprintf(vsz,"ImportFileCSV(u'%s/%s', linked=True, dsprefix=u'2_')\n",cwd,csv2file);
	fprintf(vsz,"Set('height', u'30cm')\n");
	...
	fprintf(vsz,"Add('xy', name='xy3', autoadd=False)\n");
	fprintf(vsz,"To('xy3')\n");
	fprintf(vsz,"Set('xData', u'2_rho')\n");
	fprintf(vsz,"Set('yData', u'2_Ni')\n");
	fprintf(vsz,"Set('marker', u'circle')\n");
	fprintf(vsz,"Set('markerSize', u'0.75pt')\n");
	fprintf(vsz,"Set('yAxis', u'y')\n");
	fprintf(vsz,"Set('PlotLine/hide', True)\n");
