...
! store this row
		...
		write(6,01) 'pele_row(j)=',pele_row(j)
...
!     1     abar_row, zbar_row)
      ...
      subroutine wrapper_helmeos(npart, in_den_row, &
       ...
       in_ptot_row, in_pep_row, in_xne_row, in_eta_row)
      include 'implno.dek'
      include 'const.dek'
      include 'vector_eos.dek'
      double precision &
                in_den_row,in_etot_row, &
                in_abar_row,in_zbar_row, &
                in_temp_row, in_ptot_row, &
				in_pep_row, in_xne_row, in_eta_row
      integer npart, i, j
...
!    Copy the local variables into the common block
         den_row(1)=in_den_row
         etot_row(1)=in_etot_row
         abar_row(1)=in_abar_row
         zbar_row(1)=in_zbar_row
         temp_row(1)=in_temp_row
...
!    Copy back into the energy array output
         ...
         in_etot_row=etot_row(1)
         in_ptot_row=ptot_row(1)
		 
		in_pep_row = pele_row(1) + ppos_row(1)
		in_xne_row = xne_row(1) + xnp_row(1)
		in_eta_row = etaele_row(1)
...
! xec  = hbar/kerg*e*sqrt(4*pi/me)
      ...
      parameter (third = 1.0d0/3.0d0, twoth = 2.0d0 * third, &
       pi = 3.1415926535897932384d0, avo = 6.0221367d23, &
       c = 2.99792458d10, ssol = 5.67050407222d-5, asol = 4.0d0*ssol/c, &
       zbound = 0.1d0, t7peek = 1.0d20, k2c = 4.0d0/3.0d0*asol*c, &
       meff = 1.194648642401440d-10, weid = 6.884326138694269d-5, &
       iec = 1.754582332329132d16,  xec = 4.309054377592449d-7, &
       rt3 = 1.7320508075688772d0, con2 = 1.07726359439811217d-7)


! switches for the iben & christy regimes
       double precision t6_switch1,t6_switch2
...
! switches for the conductivity
       double precision drel,drel10,drelim



! for a different interpolation
       double precision x,x1,x2,alfa,beta
...
!       drel     =  2.4d-8 * zbar/abar * temp * sqrt(temp)
