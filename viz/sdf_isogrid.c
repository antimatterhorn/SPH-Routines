...
int usage()
{
	printf("\t Creates an interpolation plot of paticles of certain isotopes on the x-y plane.\n");
	printf("\t Usage: [required] {optional}\n");
	printf("\t sdf_isogrid [sdf file] [pixels] [xmax(code units)]\n");
	return 0;
}
...
//					"windid", offsetof(SPHbody, windid), &conf,
	...
	imgSi = (double **) malloc(pixels*sizeof(double *));
	for(i=0;i<pixels;i++){
		imgSi[i]  = (double *) malloc(pixels*sizeof(double));
	}
	
	double **imgNi;
	
	imgNi = (double **) malloc(pixels*sizeof(double *));
	for(i=0;i<pixels;i++){
		imgNi[i]  = (double *) malloc(pixels*sizeof(double));
	}
	
	double **imgCO;
	
	imgCO = (double **) malloc(pixels*sizeof(double *));
	for(i=0;i<pixels;i++){
	...
	snprintf(csvfile, sizeof(csvfile), "%s_pos.csv", argv[1]);
	...
	stream = fopen(COfile,"w");
	for(i = 0; i < pixels; i++)
	{
		for(j = 0; j < pixels; j++)
		{
			if (dens[i][j] != 0) {fprintf(stream,"%f ", (double)imgCO[i][j]);}
			else {fprintf(stream,"%f ", 0.0);}
		}
		fprintf(stream,"\n");
	}
	...
	stream = fopen(Sifile,"w");
	for(i = 0; i < pixels; i++)
	{
		for(j = 0; j < pixels; j++)
		{
			if (dens[i][j] != 0) {fprintf(stream,"%f ", (double)imgSi[i][j]);}
			else {fprintf(stream,"%f ", 0.0);}
		}
		fprintf(stream,"\n");
	}
	...
	stream = fopen(Nifile,"w");
	for(i = 0; i < pixels; i++)
	{
		for(j = 0; j < pixels; j++)
		{
			if (dens[i][j] != 0) {fprintf(stream,"%f ", (double)imgNi[i][j]);}
			else {fprintf(stream,"%f ", 0.0);}
		}
		fprintf(stream,"\n");
	}
	...
	fprintf(vsz,"Set('yPos', [0.050000000000000017])\n");
	...
	fprintf(vsz,"Set('image', u'image3')\n");
	...
	fprintf(vsz,"Set('max', %f)\n",maxrho);
	fprintf(vsz,"Set('colorScaling', u'log')\n");
	fprintf(vsz,"Set('transparencyData', u'dsCO')\n");
