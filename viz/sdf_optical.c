...
int gnobj, nobj;
int conf;
double xmax,zmax,x,y,z,rr;
double rho,temp,mass,h,kappa,tau;
float tpos;
int choice,pixels;
...
double opacity_in_cm2perg;
double conductivity_in_ergperscmk;

double maxrho;

int usage()
{
	printf("\t Creates an interpolation plot of optical depth on the x-z plane.\n");
	printf("\t Usage: [required] {optional}\n");
	printf("\t sdf_opacity [sdf file] [pixels] [xmax(code units)]\n");
	return 0;
}
...
int main(int argc, char **argv[])
{
	...
	opacity_in_cm2perg			= dist_in_cm*dist_in_cm/mass_in_g;
	conductivity_in_ergperscmk	= energy_in_erg/(dist_in_cm*time_in_s*1);
...
//					"windid", offsetof(SPHbody, windid), &conf,
	...
	for(p = body; p < body+gnobj; p++)
	{
		...
		if (fabs(p->y) < p->h)		// x-z plane
		{
			...
			kappa = p->kappa * opacity_in_cm2perg;
	...
	snprintf(csvfile, sizeof(csvfile), "%s_opacity.csv", argv[1]);
...
//				if (img[i][j]/dens[i][j] > maxv) {maxv = img[i][j]/dens[i][j];}
	...
	snprintf(vszfile, sizeof(vszfile), "%s_opacity.vsz", argv[1]);
	snprintf(pdffile, sizeof(pdffile), "%s_opacity.png", argv[1]);
