	...
	for(p = body; p < body+gnobj; p++)
	{
		...
		if (r==0)
		{
			...
			if (ic >= 0 && ic < pixels && jc >= 0 && jc < pixels && kc >= 0 && kc < pixels)
			{
				...
				outArray[ic][jc][kc][0] += mass*dens_in_gccm*w(h,0);
			}
					...
					for (imk = kc-2*r; imk < kc+2*r+1; imk++) {
						if (imi >= 0 && imi < pixels && imj >= 0 && imj < pixels && imk >= 0 && imk < pixels) // inside the image
						{
							...
							{							
								...
								outArray[imi][imj][imk][2] += rho*dens_in_gccm;
								outArray[imi][imj][imk][1] += temp*mass/rho*w(h,rr*scalepix);
								outArray[imi][imj][imk][0] += mass*dens_in_gccm*w(h,rr*scalepix);
							}
						}
					}
				}
