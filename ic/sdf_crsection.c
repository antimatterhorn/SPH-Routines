...
int main(int argc, char **argv[])
{
	...
	float rho,m,h,u,abar,zbar;
	float vec[3],norm[3],nrm;
	char outfile[80];
	...
	SPHbody *body = malloc(sizeof(SPHbody) * (n));
	...
	for(i=1;i<n;i++)
	{
		theta = rand()/(double)RAND_MAX * 1 * 3.14159;
		phi = rand()/(double)RAND_MAX * 2 * 3.14159;
	...
	for(i=0;i<3;i++) vec[i] = vec[i]/nrm;
	for(i=0;i<3;i++) norm[i] = -vec[i];
...
//	snprintf(outfile, sizeof(outfile), "crsection.sdf");
//	
