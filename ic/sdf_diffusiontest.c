...
int main(int argc, char **argv[])
{
	...
	du = 1.0e16/specenergy_in_ergperg;
	r0 = 1.0;
	dr = 0.5;
	...
	for (i=0; i<nobj1; i++) {
		r = sqrt(pow(body1[i].x,2.0)+pow(body1[i].y,2.0)+pow(body1[i].z,2.0));
		if (r < r0 && r > 0.) body1[i].u += du;
	}
	
	
	
	//write the output file
	snprintf(outfile, sizeof(outfile), "diff_%s", argv[1]);
	//*fp = fopen(outfile, "w");
